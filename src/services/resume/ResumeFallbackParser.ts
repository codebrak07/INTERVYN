...
export class ResumeFallbackParser {
  static parse(resumeText: string): CandidateProfile {
  ...
  private static extractName(lines: string[]): string {
    ...
    for (const line of headerLines) {
      // Avoid headers, emails, phones, URLs
  ...
  private static extractSkills(text: string): string[] {
    ...
    for (const skill of KNOWN_SKILLS) {
      ...
      // Match full word boundary for short skills like Go, Java, C++
  ...
  private static extractExperience(lines: string[]): { role: string; company: string; duration?: string; highlights: string[] }[] {
  ...
  private static extractProjects(lines: string[], skills: string[]): { title: string; description: string; tech: string[] }[] {
  ...
  private static extractEducation(lines: string[]): { degree: string; institution: string }[] {
    const ed: { degree: string; institution: string }[] = [];
    for (const line of lines) {
